...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!password) {
      return NextResponse.json({ exists: true, user: null, access: buildMissingAccess(toolName), error: 'Contraseña requerida.' }, { status: 400 })
    }

    const response = await fetch(MIA_API_URL, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ query: LOGIN_USER, variables: { email, password } }),
      cache: 'no-store',
    })
    ...
    if (!response.ok || payload?.errors?.length) {
      const backendMessage = payload?.errors?.[0]?.message || 'No se pudo iniciar sesión.'
      const isOutdatedBackend = backendMessage.includes('Cannot query field "loginUser"')
      ...
      return NextResponse.json(
        {
          ...
          error: isOutdatedBackend
            ? 'El backend todavía no tiene activo el login con contraseña. Reinicia el MIA API y vuelve a intentar.'
